...
- **ABACマルチユーザーYCSB ベンチマーク（ワークロードC）** ✅ 新規追加

プロジェクトは本格的な運用段階にあり、各ベンチマークが独立したモジュールとして安定的に動作します。現在の主要な価値提供は、ScalarDBの性能評価における業界標準ベンチマークの実行と、特にマルチユーザー環境でのスケーラビリティ評価機能、そして**ABAC環境でのパフォーマンス評価機能**です。
